window.KTLessonEngine = (function () {
  ...
  function updateProgressBar() {
    ...
    var pct = Math.round((state.stepIndex / (state.steps.length - 1)) * 100);
  ...
  function tryResolveMatch(activity) {
    ...
    if (word === def) {
      if (window.KTAudio) KTAudio.coin();
      if (wordNode) {
